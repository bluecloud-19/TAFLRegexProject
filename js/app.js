...
`;
    ...
    document.addEventListener('keydown', e => {
      ...
      if ($('pres-mode').classList.contains('open')) {
        ...
        return; // Don't process other keys in presentation mode
      }
      
      // Modal escape
      if ($('sim-modal').classList.contains('open') && e.key === 'Escape') {
        closeModal();
        return;
      }
      
      // Main playback navigation with arrow keys
      // Only work if an automaton has been generated and not in a modal
      if (App.enfaResult && !$('sim-modal').classList.contains('open')) {
        if (e.key === 'ArrowRight') {
